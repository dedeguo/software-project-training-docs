...
- 课程验收重点是**能解释、能定位、能修改自己的代码**，而不是只会运行一个由 AI 生成的项目；
...
AI 协作记录的格式参见[文档与任务模板：模板五 AI 协作记录](../doc-templates/index.md)。
...
## AI 参与的任务
- 需求分析：AI 协助检查需求说明书的一致性，人工确认后修改 3 处业务规则；
- 编码实现：用户认证、失物管理模块由 AI 生成初稿，人工调整后提交；
- 缺陷修复：重复认领问题由 AI 协助定位根因，人工确认方案后修复；
- 测试：15 个测试用例由 AI 生成初稿，人工补充 3 个越权场景。

## 关键人工决策
- 拒绝 AI 建议引入 Redis 缓存：课程项目并发量低，引入 Redis 会增加部署复杂度；
- 修改 AI 生成的 JWT 过期时间：从 7 天改为 2 小时，符合安全要求；
- 调整 AI 生成的表结构：认领表增加审核状态字段，支持业务闭环。

## 验证方法
- 所有 AI 生成的代码均通过测试用例验证；
- 核心流程在 Docker 干净环境重新验证；
- AI 协作记录保存在 docs/ai-records/，可查阅。
```
...
```text
请先阅读当前项目的真实材料：需求分析说明书、系统设计说明书、测试报告、缺陷记录、部署说明和 AI 协作记录。

我要准备答辩 Q&A 清单。

请遵守：
1. 从技术选型、架构设计、实现细节、测试验证、部署运维、个人贡献、AI 使用七个类别，各提出 2—3 个最可能被追问的问题；
2. 每个问题给出回答要点，只使用材料中能够确认的事实；
3. 每个回答要点标注证据来源（文档、代码文件、测试报告）；
4. 不虚构成果、数据和决策过程；
5. 列出我无法用现有材料回答、需要补充准备的问题。

先说明你读取到的依据和缺失项，再生成 Q&A 清单。
```

人工必须核对：

- 每个回答要点是否与 `v1.0` 版本的实际代码和文档一致；
- AI 推测的"可能被问的问题"是否遗漏了你项目特有的难点；
